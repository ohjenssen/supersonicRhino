<script setup lang="ts">
    ...
    const exercises = ref<string[]>(JSON.parse(localStorage.getItem('exercises') ?? '[]'));
    let selectedExercise = ref<string>('squat');

    function selectExercise(exercise: string){
        selectedExercise.value = exercise.toLowerCase();
    ...
    let weight = '';
    let repetitions = '';
    const sets = JSON.parse(localStorage.getItem(selectedExercise.value) ?? "[]");
...
<template>
    <div class="exercise-selection-container">
        <button v-for="exercise in exercises" @click="selectExercise(exercise)">{{ exercise }}</button>
    </div>
    <p>Exercise selected: {{ selectedExercise }}</p>
    ...
    <!-- Squat.vue -->
     <div id="exercise">
        ...
        <form id="registration" v-bind:name="selectedExercise" @submit.prevent="handleSubmit">
